                    ...
                    <div class="border" style="border-color:#E2E8F0;">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3">
                            @foreach($header['scores'] as $label => $val)
                                ...
                                <div class="p-4 border-t sm:border-l sm:first:border-l-0"
                                     style="border-color:#E2E8F0;">
                                    <p class="text-sm font-semibold" style="color:#03314C">{{ $label }}</p>

                                    <div class="h-3 w-full overflow-hidden mt-2 border"
                                         style="background:#E2E8F0; border-color:#E2E8F0;">
                                        <div class="h-full" style="background: {{ $color }}; width: {{ $width }}%"></div>
                                    </div>

                                    <p class="text-right mt-1 text-sm font-medium" style="color:{{ $color }}">
                                        {{ $text }}
                                    </p>
                                </div>
                            @endforeach
                    ...
                    <div class="border" style="border-color:#E2E8F0;">
                        ...
                        @if(empty($qa))
                            <div class="p-4 text-sm text-gray-600">
                                No answers recorded for this evaluation.
                            </div>
                        @else
                            <ul class="divide-y" style="border-color:#E2E8F0;">
                                @foreach($qa as $row)
                                    @php
                                        // Alternate color: odd white, even light gray
                                        $rowBg = $loop->odd ? '#FFFFFF' : '#F8FAFC';
                                    @endphp

                                    <li class="p-4 space-y-1" style="background: {{ $rowBg }};">
