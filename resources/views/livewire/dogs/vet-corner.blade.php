...
<section class="max-w-7xl mx-auto mt-12 border"
    ...
    <div class="p-6">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            ...
            <span class="inline-flex items-center gap-2 text-xs font-semibold text-white px-3 py-1 border"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round"
                          d="M12 14l6.16-3.422A12.083 12.083 0 0119 13.714L12 21l-7-7.286a12.083 12.083 0 01.84-3.136L12 14z"/>
                </svg>
        ...
        <div class="border mb-10" style="border-color: {{ $KK_DIVIDER }}; background:#fff;">
            <div class="px-4 py-3"
                 style="background: {{ $KK_BLUE_ALT }}; border-bottom:1px solid {{ $KK_DIVIDER }}">
                <h3 class="font-semibold">Update metrics</h3>
            </div>

            <div class="p-6">
                <form wire:submit.prevent="saveMetrics" class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <x-ts-input label="Current weight (lb)" type="number" step="0.1" min="0"
                                wire:model.defer="current_weight" />
                    <x-ts-input label="BCS (1–9)" type="number" min="1" max="9"
                                wire:model.defer="bcs" />
                    <x-ts-input label="Next vaccine date" type="date"
                                wire:model.defer="next_vaccine_date" />
            ...
            <div class="border mb-6" style="border-color: {{ $KK_DIVIDER }}; background:#fff;">
                ...
                <div class="p-6">
                    <form wire:submit.prevent="addVisit" class="grid grid-cols-1 sm:grid-cols-4 gap-5">
                        ...
                        <x-ts-input label="Weight (lb)" type="number" step="0.1" min="0"
                                    wire:model.defer="visit_weight" />
                        ...
                        <div>
                            ...
                            @error('visit_file')
                                <p class="text-xs mt-1" style="color: {{ $KK_DANGER }}">{{ $message }}</p>
                            @enderror
                            ...
                            <div x-data="{ isUploading: false, progress: 0 }"
                                 ...
                                 class="mt-2">
                                <div x-show="isUploading" class="w-full h-2 overflow-hidden"
                                     style="background: {{ $KK_DIVIDER }};">
                                    <div class="h-full"
                                         :style="`width: ${progress}%; background: {{ $KK_BLUE }};`"></div>
                                </div>
                            </div>
                        ...
                        <div class="sm:col-span-4">
                            <x-ts-textarea label="Outcome / Notes" rows="3"
                                           wire:model.defer="outcome" />
                        </div>
                        ...
                        <tbody>
                            @forelse($dog->vetVisits as $v)
                                <tr style="border-top:1px solid {{ $KK_DIVIDER }};">
                                    ...
                                    <td class="py-2 px-3">
                                        {{ $v->weight ? number_format($v->weight,1) : '—' }}
                                    </td>
                                    <td class="py-2 px-3">
                                        @if($v->document_url)
                                            <a href="{{ $v->document_url }}" target="_blank"
                                               ...
                                               style="color: {{ $KK_BLUE }}; border-color: {{ $KK_BLUE }}; background:#fff;">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5"
                                                     ...
                                                     style="color: {{ $KK_BLUE }}">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          d="M13.5 6.75l-6.364 6.364a3 3 0 104.243 4.243l6.364-6.364a4.5 4.5 0 00-6.364-6.364L6 8.25" />
                                                </svg>
                                                View
                                            </a>
                                        @else
                                            <span style="color: {{ $KK_NAVY }}66;">—</span>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr style="border-top:1px solid {{ $KK_DIVIDER }};">
                                    <td class="py-3 px-3 text-sm" colspan="6"
